...
var $author$project$Main$count_internal = function (tree) {
	if (tree.$ === 'Node') {
		...
		var left = tree.b;
		var right = tree.c;
		return ($author$project$Main$count_internal(left) + 1) + $author$project$Main$count_internal(right);
	} else {
		var v = tree.a;
		return 0;
	}
...
var $author$project$Main$count_nodes = function (tree) {
	if (tree.$ === 'Node') {
		...
		var left = tree.b;
		var right = tree.c;
		return ($author$project$Main$count_nodes(left) + 1) + $author$project$Main$count_nodes(right);
	} else {
		var v = tree.a;
		return 1;
	}
	...
	function (s, slist) {
		...
		while (true) {
			if (!slist.b) {
				...
				var xs = slist.b;
				if (_Utils_eq(x, s)) {
					return 1 + A2($author$project$Main$count_occurrences, s, xs);
				} else {
					var $temp$s = s,
						$temp$slist = xs;
					s = $temp$s;
					slist = $temp$slist;
					continue count_occurrences;
				}
			}
		}
	});
			...
			if (!path.b) {
				...
				if (path.a.$ === 'Left') {
					...
					return A3(
						$author$project$Defns$Node,
						v,
						A4($author$project$Main$tree_insert, xs, lst, rst, left),
						right);
				} else {
					var _v3 = path.a;
					var xs = path.b;
					return A3(
						...
						v,
						left,
						A4($author$project$Main$tree_insert, xs, lst, rst, right));
				}
			}
		} else {
			var v = tree.a;
...
var $author$project$Main$invert = function (twolst) {
		...
		if (!_v0.a.b) {
			...
			if (!_v0.b.b) {
				...
				return (_Utils_cmp(x, y) < 1) ? A2(
					$elm$core$List$cons,
					x,
					A2($author$project$Main$merge, xs, lst2)) : A2(
					$elm$core$List$cons,
					y,
					A2($author$project$Main$merge, lst1, ys));
			}
		}
	});
...
var $author$project$Main$product = F2(
	function (lstA, lstB) {
		...
		var _v2 = _Utils_Tuple2(lstA, lstB);
		if (!_v2.a.b) {
			return _List_Nil;
		} else {
			if (!_v2.b.b) {
				...
				var itemsA = _v2.a;
				var itemsB = _v2.b;
				return A2(product2, itemsA, itemsB);
			}
		}
	});
	...
	function (key, tree) {
		if (tree.$ === 'Node') {
			...
			if (_Utils_eq(v, key)) {
				...
				var _v1 = A2($author$project$Main$search, key, left);
				if (_v1.$ === 'Nothing') {
					var _v2 = A2($author$project$Main$search, key, right);
					if (_v2.$ === 'Nothing') {
						return $elm$core$Maybe$Nothing;
					} else {
						var items = _v2.a;
						return $elm$core$Maybe$Just(
							A2($elm$core$List$cons, $author$project$Defns$Right, items));
					}
				} else {
					var items = _v1.a;
					return $elm$core$Maybe$Just(
						A2($elm$core$List$cons, $author$project$Defns$Left, items));
				}
			}
		} else {
			var v = tree.a;
			return _Utils_eq(v, key) ? $elm$core$Maybe$Just(_List_Nil) : $elm$core$Maybe$Nothing;
		}
	});
	...
	function (f, tree) {
		if (tree.$ === 'Node') {
			...
			return A3(
				$author$project$Defns$Node,
				f(v),
				A2($author$project$Main$tree_map, f, left),
				A2($author$project$Main$tree_map, f, right));
		} else {
			var v = tree.a;
			...
			if (!path.b) {
				...
				if (path.a.$ === 'Left') {
					...
					return A3(
						$author$project$Defns$Node,
						v,
						A3($author$project$Main$update, xs, f, left),
						right);
				} else {
					var _v3 = path.a;
					var xs = path.b;
					return A3(
						...
						v,
						left,
						A3($author$project$Main$update, xs, f, right));
				}
			}
		} else {
			var v = tree.a;
...
var $author$project$Test$Generated$Main1709255165$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/home/yashas/Desktop/semester/popl/popl-assignments/hws/hw02/tests/SampleTests.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 342473728465952
	},
...
_Platform_export({'Test':{'Generated':{'Main1709255165':{'init':$author$project$Test$Generated$Main1709255165$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-595318.sock";
